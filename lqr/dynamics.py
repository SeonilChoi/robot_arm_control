...
def linearize(q_e, eps=1e-7):
    """Return A (6x6), B (6x3) at equilibrium z_e = [q_e, 0], tau_e = G(q_e)."""
    tau_e = G(q_e)
    ...
    for i in range(nq):
        q_plus = q_e.copy()
        q_plus[i] += eps
        qdd_plus = qdd_from_tau(q_plus, np.zeros(3), tau_e)
        q_minus = q_e.copy()
        q_minus[i] -= eps
        qdd_minus = qdd_from_tau(q_minus, np.zeros(3), tau_e)
        A[3:6, i] = (qdd_plus - qdd_minus) / (2*eps)
    for i in range(nqd):
        qd_eps = np.zeros(3)
        qd_eps[i] = eps
        qdd_plus = qdd_from_tau(q_e, qd_eps, tau_e)
        qdd_minus = qdd_from_tau(q_e, -qd_eps, tau_e)
        A[3:6, 3+i] = (qdd_plus - qdd_minus) / (2*eps)
    A[0:3, 3:6] = np.eye(3)
    return A, B
